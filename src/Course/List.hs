...
length ::
  List a
  -> Int
--length = foldRight (P.flip (P.const . P.succ)) 0
length = foldLeft (P.const . (+1)) 0
...
--seqOptional = foldRight ((P.<*>) . (P.fmap (:.))) $ Full Nil
--seqOptional = foldRight (\x acc -> (:.) P.<$> x P.<*> acc) $ Full Nil
